...
from  ml_model import create_vdata,creater
...
@second.route("/save_video_sentiments")
...
@second.route('/save_video_sentiments', methods=['POST','GET'])
def save_video_sentiments():
    url = request.form.get("url")
    print(url)
    #url="https://www.youtube.com/watch?v=bMLbnsKGRfo&ab_channel=MarquesBrownlee"
...
#     with open("data/video_title_sentiments.json", "r") as f:
#         data = json.load(f)
...
#     r = n_neg / (n_pos + n_neg+ n_neut)
#     print(f"Negative ratio: {r:.3f}")a

#     r = n_neut / (n_pos + n_neg+ n_neut)
#     print(f"Neutral ratio: {r:.3f}")
#     return render_template('sentiment_analyzer.html',r=r)
    creater()
    r_pos,r_neg,r_neut,n_pos,n_neg,n_neut,polarity_score=create_vdata()
    ...
    return render_template('sentiment_analyzer.html',rp=r_pos,rn=r_neg,rne=r_neut,np=n_pos,nv=n_neg,nneu=n_neut,polarity=polarity,pl=polarity_score,videoId=video_id)
# # save_video_sentiments("https://www.youtube.com/watch?v=bMLbnsKGRfo&ab_channel=MarquesBrownlee")
